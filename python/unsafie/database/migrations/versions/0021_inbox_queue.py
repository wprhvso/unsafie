...
def upgrade() -> None:
    op.execute(
        "ALTER TABLE updates ADD COLUMN IF NOT EXISTS status VARCHAR(16) NOT NULL DEFAULT 'pending'"
    )
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_updates_pending ON updates (id ASC) WHERE status = 'pending'"
    )
