...
def prompt_markup(locale: str) -> InlineKeyboardMarkup:
    buttons = [
        InlineKeyboardButton(
            text=f"✅ {t('cmd-long-send', locale)}",
            callback_data=LongCallback(choice=LongChoice.SEND.value).pack(),
        ),
        InlineKeyboardButton(
            text=f"🗑 {t('cmd-long-reset', locale)}",
            callback_data=LongCallback(choice=LongChoice.RESET.value).pack(),
        ),
    ]
...
class LongInputService:
    def __init__(self, timeout: float = DEFAULT_TIMEOUT) -> None:
        ...
        self._collections: dict[tuple[int, int], _Collection] = {}

    def collecting(self, bot_id: int, chat_id: int) -> bool:
        return (bot_id, chat_id) in self._collections

    def target_of(self, bot_id: int, chat_id: int) -> LongTarget | None:
        col = self._collections.get((bot_id, chat_id))
    ...
    async def start(
        ...
        await self.clear(bot_id, chat_id)
        col = _Collection(target=target, locale=locale)
        ...
        markup = prompt_markup(locale)

        sent = await bot.send_message(chat_id=chat_id, text=text, reply_markup=markup)
        col.prompt_message_id = sent.message_id
        col.expiry = asyncio.create_task(self._expire(bot, bot_id, chat_id, locale))
        self._collections[(bot_id, chat_id)] = col
    ...
    async def append(
        ...
        if col.expiry:
            col.expiry.cancel()
        col.expiry = asyncio.create_task(self._expire(bot, bot_id, chat_id, col.locale))
    ...
    async def clear(self, bot_id: int, chat_id: int) -> None:
        col = self._collections.pop((bot_id, chat_id), None)
        if col and col.expiry:
            col.expiry.cancel()

    async def _expire(self, bot: Bot, bot_id: int, chat_id: int, locale: str) -> None:
