...
from unsafie_cli.errors import FAILED, OK
    ...
    def read(self, text: str) -> None:
        with self.lock:
            if LISTENING in text and not self.listening:
                self.listening = True
                self.out.line("[runner] listening for jobs")
            elif STARTED in text:
                self.job = text.split(STARTED, 1)[1].strip()
                self.jobs += 1
                self.out.line(f"[runner] job started: {self.job}")
            elif FINISHED in text and self.job:
                result = text.rsplit(FINISHED, 1)[1].strip()
                self.results.append(result)
                self.out.line(f"[runner] job finished: {self.job} -> {result}")
                self.job = ""
...
def _watchdog(
    ...
    while not done.wait(POLL):
        ...
        if not (overdue or unused):
            continue
        progress.out.line(f"[runner] stopping: {'lifetime' if overdue else 'idle'}")
...
def run_runner(
    ...
    progress = Progress(speaker)
    done = threading.Event()
    speaker.line(f"[runner] {name or 'runner'} for {repo or 'a repository'} on v{version}")
    process = None
    try:
