...
class Daemon:
    ...
    def facts(self) -> dict[str, Any]:
        ...
        return {
            ...
            "region": os.environ.get("RUNNER_ENVIRONMENT") or "",
        }
    ...
    def _environment(self) -> dict[str, str]:
        env = dict(os.environ)
        env["UNSAFIE_API"] = self.link.base
        env["UNSAFIE_MACHINE"] = self.name
        ...
        env.pop("UNSAFIE_WORKER_TOKEN", None)
        return env
    ...
    def _execute(self, raw: dict) -> None:
        ...
        if process.stdin is not None:
            try:
                if stdin:
                    process.stdin.write(str(stdin).encode())
                process.stdin.close()
            except OSError:
                pass
        watchdog = None
...
def serve(api: str, worker_token: str, profile: str = "fast") -> int:
    link = Link(api, worker_token)
    daemon = Daemon(link, profile)
    return daemon.serve()
