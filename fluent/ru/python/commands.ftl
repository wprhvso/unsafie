...
commands-start =
    ...
    /wipe — удалить всю историю диалогов, файлы песочницы и артефакты
    /stop — остановить: реплаем на сообщение — то, что запустило оно, без реплая — вообще всё
    /pipe — последовательное выполнение строк отдельными ходами
    /effort — усилие на размышления
    /load — выгрузить файлы репозитория в контекст
...
commands-wipe-success = Вся история диалогов, контекст, файлы песочницы и артефакты для этого чата безвозвратно удалены.

commands-load-usage = Использование: /load owner/repo [owner2/repo2 ...]
commands-load-success =
    Успешно загружено { $chars } символов из { $repos } ({ $files } файлов).
    Ответьте на это сообщение, чтобы продолжить диалог с контекстом.
commands-load-all-failed =
    Не удалось загрузить репозитории:
    { $errors }
commands-load-partial-failed = Не удалось загрузить: { $failed }
