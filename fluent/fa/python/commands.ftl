...
commands-start =
    ...
    /wipe — حذف تمام سوابق گفتگو، فایل‌های سندباکس و دست‌سازه‌ها
    /stop — توقف: در پاسخ، آنچه این پیام آغاز کرده؛ بدون پاسخ، همه چیز
    /effort — تلاش تفکر
    /load — بارگذاری فایل‌های مخزن در زمینه
...
cmd-system-help =
    ...
    نمونه‌ها:
    `/system تا جای ممکن کوتاه پاسخ بده`
    `/system در پاسخت از اصطلاحات فنی استفاده نکن`
...
cmd-runs-empty = هیچ کار فعالی در این چت وجود ندارد

commands-wipe-success = تمام سوابق گفتگو، زمینه، فایل‌های سندباکس و دست‌سازه‌های این گفتگو برای همیشه حذف شدند.

commands-load-usage = استفاده: /load owner/repo [owner2/repo2 ...]
commands-load-success =
    { $chars } کاراکتر با موفقیت از { $repos } ({ $files } فایل) بارگذاری شد.
    برای ادامه گفتگو با زمینه مخزن به این پیام پاسخ دهید.
commands-load-all-failed =
    بارگذاری مخازن ناموفق بود:
    { $errors }
commands-load-partial-failed = بارگذاری این موارد ناموفق بود: { $failed }
